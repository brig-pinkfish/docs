...
Using Pinkfish's [Queue and Batch services](/orchestration/queue), you can orchestrate these three workflows to handle any size batch efficiently.
...
```
/batch — Post new batch (each input is the chunk filename)
Set worker to Consumer, onDone Trigger to Aggregator
See [Batch](/orchestration/batching) for full setup.
```
...
For more details on Queue and Batch service configuration, see the [Queue Service documentation](/orchestration/queue).
