...
### 2. Attach an API Trigger (to your worker automation)
As part of the item creation priocess, attach a trigger to another of your automations. The automation that you trigger will be your worker automation. It will receive a payload containing details about the job (the item's content and metadata).

### 3. Update Status to kick off your worker automation
When ready to process, update the item's status (that you created in step 1). This change will automatically fire your trigger and will kick off your worker automation.
