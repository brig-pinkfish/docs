---
title: "pinkfish workflows"
sidebarTitle: "pinkfish workflows"
...
| [`workflow_update`](#workflow_update)         | Update workflow code                                       |
| [`workflow_bind_resources`](#workflow_bind_resources) | Bind declared workflow resources to concrete IDs |
...
Update workflow code only. Save code first, then call `workflow_bind_resources` to apply bindings.
...
| `code`              | string | Yes      | Workflow code (full replacement or targeted edit format) |
...
Bind declared workflow resources to concrete resource IDs after saving code with `workflow_update`.
...
| -------------- | ------ | -------- | ---------------------------------------------------------------------------- |
| `automationId` | string | Yes      | Automation ID                                                                |
| `bindings`     | object | Yes      | Resource bindings map keyed by `WORKFLOW_RESOURCES` name                     |
...
```json
{
  ...
  "properties": {
    ...
    "bindings": {
      "type": "object",
      "description": "Bindings keyed by resource name",
      "additionalProperties": {
        "type": "object",
        "properties": {
          "id": { "type": "string", "description": "Concrete resource ID" },
          "resourceType": {
            "type": "string",
            "enum": ["connection", "collection", "trigger", "agent"],
            "description": "Bound resource type"
          },
          "subtype": { "type": "string", "description": "Optional subtype for collection-like resources" },
          "description": { "type": "string", "description": "Optional binding note" }
        },
        "required": ["id", "resourceType"]
      }
    }
  },
  "required": ["automationId", "bindings"]
}
```
...
**Creating an agent with tools:** Pass the `servers` parameter on create (or update). Each server has a `name` (e.g. `"slack"`, `"gmail"`) and a `tools` object mapping tool names to configs — use `{}` for defaults. Tool names include the server prefix (e.g. `"slack_send_message"`). Use `capabilities_discover` to find valid server and tool names.
