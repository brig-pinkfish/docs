...
1. **Poll for results programmatically** (recommended for integrations). See the [Polling for Results](#polling-for-results) section below.
...
You can use these values to poll for the run status and retrieve results once the workflow completes.

**Note**: Polling endpoints use the webhook-style URL format (`/ext/webhook/{apiKey}/...`) even if the run was started using the API trigger endpoint with `X-API-KEY` header authentication.
...
The status endpoint returns a `RunStatus` object:
...
Returns the complete run object with all step results (same format as synchronous response):
...
If your trigger has a designated output configured, the async results response includes a top-level `designatedOutput` object.
