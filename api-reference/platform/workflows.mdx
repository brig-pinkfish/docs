...
All workflow tools are on the `/pinkfish-sidekick` server path. For full parameter schemas, see the [pinkfish workflows server reference](/api-reference/mcp-servers/embedded/pinkfish-sidekick).
...
| `workflow_update`              | Update workflow code only                                                                                          |
| `workflow_bind_resources`      | Bind declared connections, triggers, agents, and collections after saving code                                     |
...
curl -s -X POST "https://mcp.app.pinkfish.ai/pinkfish-sidekick" \
  ...
  -d '{
    ...
    "method": "tools/call",
    "params": {
      "name": "workflow_bind_resources",
      "arguments": {
        "automationId": "auto_abc123",
        "bindings": {
          "zendesk": {
            "id": "<YOUR_ZENDESK_PCID>",
            "resourceType": "connection"
          },
          "ticketAnalyst": {
            "id": "<YOUR_AGENT_ID>",
...
4. workflow_update(automationId, code: "...full workflow JS...")
   -> Workflow saved with node graph

5. workflow_bind_resources(automationId, bindings: {...})
   -> Declared resources are mapped to concrete IDs

6. workflow_read(automationId, checkResources: true)
   -> Confirms binding validity before execution

7. workflow_trigger_schedule(automationId, schedule: "0 9 * * *")
   -> Trigger created — workflow fires daily at 9am

8. workflow_run(automationId)
   -> Test execution — verifies everything works

9. workflow_results(automationId, operation: "read", filename: "output.json")
