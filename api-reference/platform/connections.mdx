...
2. **A PCID** — Every connection has a unique PinkConnect ID. You pass this PCID as a tool argument so the [MCP Farm](/api-reference/platform/mcp-protocol) knows which OAuth connection to use (e.g., which Gmail account).

See the [Application MCPs overview](/api-reference/mcp-servers/application/overview) for the full list of supported services.
