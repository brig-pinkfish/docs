...
5. **workflow_bind_resources** — Bind any newly declared connections, triggers, agents, or
   collections after saving code. This tool owns all manual binding and rebinding.
6. **workflow_read** — Call `workflow_read({ automationId, checkResources: true })` to verify
   every declared resource is valid before running.
7. **workflow_run** — Execute and verify. Use workflow_results to inspect output when you need
   to search, read, or get URLs from run artifacts.
8. **workflow_pin** — IMMEDIATELY pin all mcp-tool nodes after successful run. Pinned nodes
   skip execution and reuse stored output; unpinned nodes re-run every time.
9. **Fix & iterate** — Review errors, update code, run again until success. Call workflow_read
...
For lifecycle: workflow_create, workflow_update, workflow_bind_resources,
  workflow_read, workflow_run, workflow_run_status, workflow_set_inputs,
  workflow_pin, workflow_results, workflow_list
