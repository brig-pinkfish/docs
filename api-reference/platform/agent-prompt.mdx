...
When you connect an AI agent to the Pinkfish MCP Farm, it receives tools it can call. This page provides the system prompt that teaches the agent _when_ and _how_ to use them.

For **workflow-building agents**, see [Workflow Builder Prompt](/api-reference/platform/workflow-builder-prompt).

For **programmatic agents** (scripts, custom code) that call the API over HTTP, see [Discover, Install, and Invoke](/api-reference/platform/dynamic-tool-invocation).
...
<Note>
  **MCP vs HTTP:** When an agent is connected via Cursor, an IDE, or a custom
  MCP client, it receives tools it can invoke directly. For programmatic agents
  that call the API over HTTP, see [Dynamic Tool
  Discovery](/api-reference/platform/dynamic-tool-invocation).
</Note>
