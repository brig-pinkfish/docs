...
The Platform API uses API keys for authentication. Exchange an API key for a short-lived runtime token (JWT), then use that token as a Bearer token for all MCP Farm requests.

**Token lifetime:** 60 minutes. For long-running agents, call `/auth/token` again before expiry.
...
<Warning>
New API keys can take up to 60 seconds to activate.
</Warning>
