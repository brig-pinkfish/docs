...
<Note>The token is for **MCP requests only**. App API endpoints (e.g. `/api/automations`) require Cognito login and do not accept this token.</Note>
...
Use the token for MCP server requests. The server path depends on your MCP integration.

```bash
curl -X POST "https://mcp.app.pinkfish.ai/<server-path>" \
