...
### Success: Structured Content vs Legacy

Tool responses support two formats. **Structured content** is the new standard; both are supported for compatibility.

| Format | Field | Description |
|--------|-------|-------------|
| **Structured content** (preferred) | `result.structuredContent` | Type-safe object matching the tool's `outputSchema`. No parsing needed. |
| **Legacy** | `result.content[0].text` | JSON string. Parse with `JSON.parse()` for structured data. |

**Default (no header):** Responses include both fields. Prefer `structuredContent` when present; fall back to `content[0].text` for older integrations.

**`X-MCP-Mode` header (optional):**

| Header | Response |
|--------|----------|
| `X-MCP-Mode: optimized` | `structuredContent` only — for agents and agentic workflows |
| `X-MCP-Mode: legacy` | Legacy format only — for guided workflows |
| No header | Both `content` and `structuredContent` |

**Example — prefer structured, fall back to legacy:**
...
Responses include `error.code` and `error.message`:
