...
The MCP Farm uses **JSON-RPC 2.0 over HTTPS**. Every call is an HTTP POST to a server path, with a JSON-RPC payload in the body.
...
**Error:** `error.code` and `error.message` describe the failure.
