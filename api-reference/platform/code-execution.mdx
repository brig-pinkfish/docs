...
The MCP Farm includes a sandboxed JavaScript execution environment that can orchestrate multi-step logic, chain tool calls, and transform data programmatically. This is useful when you need to perform complex operations that go beyond a single tool call.

The code execution server is at `/restricted/code-execution`.
...
await createArtifact("report.json", JSON.stringify(data, null, 2), "application/json");
...
| Persistent, versioned automation | [Workflows](/api-reference/platform/workflows) |
| Scheduled or triggered automation | [Workflows](/api-reference/platform/workflows) |

For simple, single tool calls, call the tool directly. For persistent automations with triggers, scheduling, and visual editing, use [Workflows](/api-reference/platform/workflows).
