...
When you don't know which tool to use: call `capabilities_discover` with a task description, then `capability_details` for schemas, then `tools/call` on the recommended server path. Both discovery tools are on `/pinkfish-sidekick`.
