...
2. **Agent discovers what it needs** — `capabilities_discover` returns tool names, server
   paths, and PCIDs.
3. **Agent gets the schema** — `capability_details` returns the full `inputSchema`
   (standard JSON Schema) and `serverName` for each tool.
4. **Your code converts and registers** — Map the returned schema to your model's
...
this approach uses `capability_details` to get full `inputSchema` definitions for only
the specific tools the agent discovered — typically 1–5 tools instead of hundreds.
For external integrations, include the `PCID` from the discovery response in the tool
call arguments.
</Note>
