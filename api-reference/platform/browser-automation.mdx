...
The `browser-automation` MCP server provides AI-powered browser automation using natural language or Playwright code. Describe what you want in plain English and watch the AI navigate, click, fill forms, and extract data — with automatic file capture for extracted data, downloads, and screenshots.

## Why This Replaces RPA

| | Traditional RPA | Pinkfish Browser Automation |
|---|---|---|
| **Task definition** | Fragile selectors and recorded click paths | Natural language instructions |
| **Resilience** | Breaks when UI changes | AI adapts to layout changes — finds elements by intent, not selectors |
| **Maintenance** | Requires script updates per UI change | Describe what you want, not how to find it |
| **CAPTCHA / Anti-bot** | Requires separate tooling | Built-in CAPTCHA solving and stealth mode |
| **Visibility** | Logs and screenshots after the fact | Live visualization via URL |
| **File output** | Custom export scripting | Automatic capture — extracted data, downloads, and screenshots returned as files |
| **Infrastructure** | Desktop agents, VM farms | Cloud-hosted browsers — no agents to install |
| **Coding** | Proprietary scripting languages | Optional Playwright (JavaScript) for precise control |
...
**Default approach:** Use Browser Operator (natural language) for everything. Only use Playwright when you need exact programmatic control.

## Browser Operator: Natural Language Automation
...
async function node_scrape_data(params) {
  ...
  let result = await pf.mcp.callTool('browser-automation', 'browser-automation_operator_run', {
    task: params.task,
    cacheKey: 'a1b2c3d4',
    model: 'google/gemini-3-flash-preview',
    agentMode: 'hybrid',
  ...
  while (result.status !== 'completed' && result.status !== 'failed') {
    ...
    result = await pf.mcp.callTool('browser-automation', 'browser-automation_operator_run_continue', {
