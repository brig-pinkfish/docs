...
Simply use the command followed by the type of chart you want. Give it reference to a dataset that you've prepared in a previous step or a file that you've uploaded to your automation:

```
/D3 create bar chart using SALES_2023_DATA, show months on x-axis and revenue on y-axis
```
...
```
add in /CSV skill to do a better job at parsing. i only see one country with data. 
```
...
Here's a link to my sample dataset that you can use to experiment. 
[Customers-100.csv](/images/sampledata/customers-100.csv)
