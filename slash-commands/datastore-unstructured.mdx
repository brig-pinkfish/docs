...
- **Natural language search** - search and ask questions about your data in plain English
- **Pagination** - offset-based pagination for large datasets (500 items per page)
- **Server-side sorting** - sort by key, sortField, createdAt, or updatedAt
...
- `orderedBy` - Sort by key, sortField, createdAt, or updatedAt (e.g., "createdAt:desc", "key:asc")
...
To page through results, increment offset by limit (e.g., offset=0, offset=500, offset=1000).
...
**Put mutable data in content or metadata:**

Both `content` and `metadata` fields are fully mutable and can be updated at any time.
...
Metadata fields can be filtered using `metadata.fieldName=value` in queries, making them ideal for attributes you need to search or filter by.
...
Automatically start workflows when data changes in your unstructured datastore. Triggers can be set at both collection-level (all items) and item-level (specific items).
