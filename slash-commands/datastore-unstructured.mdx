...
Updates are partial - only specified fields change:
...
Automatically start workflows when data changes.

### Trigger Levels

**Collection-Level**: Monitor all items in the collection
- Set via collection management UI
- Perfect for system-wide monitoring
- Events: onCreate, onEdit, onDelete

**Item-Level**: Monitor specific items only
- Set via `triggerUrls` parameter
- Perfect for business-critical data
- Events: onEdit, onDelete (onCreate not available at item level)
...
**onEdit (includes changed field):**
```json
{
  "event": "onEdit",
  "itemId": "abc123",
  "content": {"theme": "light"},
  "changed": {"content": {"theme": "dark"}},
  "triggerLevel": "item"
}
```
...
- onEdit triggers: 2 executions per minute per item (prevents infinite loops)
- All executions logged with status and timing
- Failed triggers logged but don't block data operations
...
- Updates are partial - only sent fields are modified
