...
The `/perplexity` command supports processing various document formats through the `file_urls` parameter:
...
<Note>
Images are not supported by Perplexity. Only the document formats listed above can be processed.
</Note>
