...
- `language_hints` - Array of language codes for better OCR accuracy (e.g., ["en", "es"])
- `extract_text_only` - When true (default), returns only the extracted text. When false, includes detailed OCR data like bounding boxes, word positions, page structure, and text annotations for precise layout analysis

### Response Format

**Basic Response (extract_text_only: true):**
```json
{
  "success": true,
  "capability": "ocr",
...
**Detailed Response (extract_text_only: false):**
```json
{
  "success": true,
  "capability": "ocr",
