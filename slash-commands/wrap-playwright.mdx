---
title: "Wrap Playwright Guide (Deprecated)"
...
1. Find your file using `config.findUploadedFile('filename.pdf', { exactMatch: true })`
2. In your Playwright code, fetch the file from its URL and create a buffer
3. Create a file payload object: `{ name: 'filename.pdf', mimeType: 'application/pdf', buffer: buffer }`
4. Use `page.locator('#file-input').setInputFiles(filePayload)` to attach to the input
