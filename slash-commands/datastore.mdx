...
**For Structured Collections:**
Content must be an object with field IDs (e.g., `{"f_1": "John Doe", "f_2": "john@example.com"}`). To discover field IDs, list one existing item or fetch the collection schema.

**For Unstructured Collections:**
Content can be any type: string, number, object, or array.
...
Structured (using field IDs):
```
/selected-datastore create a new item with:
key: customer-456
content: {"f_1": "Jane Smith", "f_2": "jane@company.com", "f_3": "(415) 555-0100"}
```
...
- Recommended batch size: Up to 100 items for optimal performance (larger batches may cause gateway timeouts)
...
Store, retrieve, update, and search data in two modes:

**Structured Database Tables**: Spreadsheet-like databases with defined columns, schema validation, and inline editing. Perfect for customer lists, inventory, contacts, and any structured records.

**Unstructured Databases**: Flexible JSON storage for any data type. Perfect for logs, API responses, configuration, and dynamic content that doesn't fit a fixed schema.
...
## Working with Structured Collections

**Field IDs:**
- Structured collections use short field IDs (f_1, f_2, f_3) instead of column names
- Field IDs are stable - column labels can change but IDs never change
- To discover field IDs: list one item and examine the content structure

**Learning Field Structure:**
```
/selected-datastore list items with limit: 1
```
Response shows: `{"content": {"f_1": "Alice", "f_2": "alice@email.com", "f_3": "(415) 555-0101"}}`

Now you know f_1, f_2, f_3 are the field IDs to use for new items.

**Schema Management:**
- Add columns via UI (+ New Column button)
- Rename columns by double-clicking header
- Delete columns via column menu
- Schema changes don't require updating existing rows
...
- For structured collections, learn field IDs from existing data to avoid extra API calls
