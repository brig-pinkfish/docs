...
Store, retrieve, update, and manage files in the cloud storage system. Supports text and binary files with metadata, link expiration control, and public file sharing options.
...
```
/filestorage
action: upload, get, list, update, or delete
filename: name of the file
content: file content or data
```

### Parameters
- **action**: Operation to perform (upload, get, list, update, delete)
- **filename**: Name of the file (required for most operations)
- **content**: File content for upload/update
- **metadata** (optional): Additional information about the file
- **expiration** (optional): Link expiry (1-7 days or "never" for public)
- **collection** (optional): Collection ID for file organization

### Response Format
Returns file URLs, metadata, and signed URLs for access. List operations return arrays of file information.
...
### Specific Use Case
List and extract content:
```
/filestorage
action: list all files with content extraction
format: detailed
extract: pdf, docx, xlsx files
```

### Notes
- Supports text and binary files (images, PDFs, documents, etc.)
- Files can be made public with expiration="never"
- Use PATCH for updating binary files without re-uploading
- Automatic content extraction for PDFs, Word, Excel, PowerPoint
- Maximum link expiration is 7 days (unless public)
