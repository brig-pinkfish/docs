...
There is no race condition prevention in place. If you have multiple workflows that are trying to write to the same file, you will experience unexpected results.
...
In the left nav, click "File Storage" where you'll see a list of files that you've created through your workflows or directly via the UI. Click Upload to upload a new file.
...
Perhaps you want to store an email template for use across your workflows. You could write a file with the following slash command:
...
Now you can use the file in your workflows. To retrieve it you would write a prompt like this:
`/filestorage retrieve contents of file welcome-email-template.json from file storage`

Note that the reason I set file_links_expire_in_days to 1 is because that expiry is related to the signed URL that Pinkfish generates for you to access the file. And I only need it to be accessible for the duration of the Workflow run.
