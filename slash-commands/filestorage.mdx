...
The parameters are appended to both the signed URL and the short URL:
```
file_url:  https://skills.pinkfish.ai/files/{collectionId}/app.html?signed-key=xxx&id=123&mode=preview
short_url: https://skills.pinkfish.ai/files/s/aBcD1234xY?id=123&mode=preview
```
...
The `short_url` is a compact alias that serves the same file as `file_url`. It is easier to share and more reliable for LLMs to reproduce. Short URLs have the same expiration as the signed URL they were created alongside. Public files (expiry set to "never") do not include a short URL since they already have clean URLs.
...
```json
{
  ...
  "isPublic": false,
  "signedUrl": "https://...",
  "shortUrl": "https://skills.pinkfish.ai/files/s/aBcD1234xY",
  "accessList": [
    {
      "action": "create",
      "providerId": "user-id",
...
- **Full details**: All metadata, signedUrl, shortUrl, and accessList
...
- **Short URLs**: Non-public files also return a `short_url` — a compact alias (e.g., `https://skills.pinkfish.ai/files/s/aBcD1234xY`) that serves the same content as the signed URL. Short URLs share the same expiration as the signed URL and are easier to share and reproduce
