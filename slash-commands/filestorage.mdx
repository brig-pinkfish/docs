...
```
file_url:  https://skills.pinkfish.ai/files/{collectionId}/app.html?signed-key=xxx&id=123&mode=preview
```

For share links, query parameters are stored in the database and appended via a 302 redirect when the short URL is accessed. The short URL itself remains clean:
```
short_url: https://skills.pinkfish.ai/files/s/aBcD1234xY
→ redirects to: https://skills.pinkfish.ai/files/s/aBcD1234xY?id=123&mode=preview
```
...
Supported on: **create**, **get**, **list**, and **sharelink** operations.
...
```json
{
  ...
  "file_url": "https://skills.pinkfish.ai/files/{collectionId}/file.txt?signed-key=..."
}
```
...
```json
{
  ...
  "queryParams": {"id": "123", "mode": "preview"}
}
```
...
Create a short, shareable URL for a file with a specified expiration. Each call generates a unique short URL. Optionally store query parameters that are appended to the URL via redirect when accessed.

**Parameters:**

- `action` (required) - Set to "sharelink"
- `filename` (required) - Name of the file
- `file_links_expire_in_days` (optional) - Expiry: 1-7 days. At least one of days or minutes is required
- `file_links_expire_in_minutes` (optional) - Minute-level precision
- `query_params` (optional) - JSON key-value pairs stored with the link and appended via redirect on access

**Examples:**

Create a share link with 1-day expiry:

```
/selected-filestore create share link:
filename: report.pdf
file_links_expire_in_days: "1"
```

Create a share link with stored query params:

```
/selected-filestore create share link:
filename: dashboard.html
file_links_expire_in_days: "7"
query_params: {"id": "customer-123", "mode": "readonly"}
```

Multiple share links for the same file are supported — each has its own unique short code and independent expiry.
...
For complete documentation on setting up triggers, payload formats, rate limiting, and best practices, see the [Event Triggers guide](/triggers/event-triggers).
...
- **Share Links**: Use the `sharelink` action to explicitly create short, shareable URLs for files. Each share link has its own unique short code and independent expiry. Query parameters can be stored with the link and are appended via 302 redirect when the short URL is accessed. Share links are not automatically generated — they must be explicitly created