...
I've been talking about _user ids_ for long term memory, but how the heck do you get a userId? Simple, you make it up. It could be "123" or it could be the user's email address or phone number (easier). And then, if you want your agent to be aware of memories for a given user, just send in the userId and useMemory flag. In the prompt below, you can see I added two additional parameters at the end to let the agent know that we want to use memory for this interaction.
