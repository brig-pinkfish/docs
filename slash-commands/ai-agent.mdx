...
If you want to have an ongoing conversation with a user, where the agent remembers what happened previously and responds in a human-like way, an AI Agent is a good option. Or if you want the agent to be able to "think" by itself and figure out how to complete a task given only an objective and a set of tools, an AI Agent is a good fit. The promise of AI Agents is that with relatively simple instructions, you can create a highly autonomous agent that can perform complex tasks. The hardest part about building an agent is giving it tools to perform the various tasks you want it to do. With Pinkfish, this part is easy peasy (build automations as tools). So it's a great platform for building AI Agents.

However, if you want to create an automation that receives an inbound SMS or email, processes that message in some way, and then responds to the user, you don't need an AI Agent. This is a simple automation that receives an input, processes that input, and then responds using the same channel as the inbound message. An AI Agent is overkill. If you are interfacing with a critical system and you cannot afford to make mistakes, don't use an AI Agent. You need a deterministic automation that you can test and publish - having it work the same way every time. Just build a simple multi-step automation for these cases.
...
The threadId is a unique identifier for tracking a conversation over time. Your threadId could simply be the user's phone number or email address. Or you could create a step to generate a unique identifier that you then use as the threadId.

The format we like for thread IDs is `[userId]-[uniqueId]` - but really, it's entirely up to you. The most important thing is that you _**get the threadId back in the response from the user**_. For this reason, using a phone number or email address is the simplest solution. If you are using these channels, the sender's phone number or email address is already unique and will be sent by the user with every message. The downside to using a phone number or email address is that the thread will be locked into that user. So if the user cc's other people and they reply, the sender email will change and you'll lose the thread.

## Optional Fields

### messageHistory

This parameter is useful if you want to provide a custom message history for testing or debugging.

- Overrides retrieval of message history
- You must still include threadId even when providing custom history (even though that threadId history will be ignored)

## Typical Step Structure

A typical agent automation consists of three steps: getting the inputs, calling the agent, and responding to the user. Here's a sample:
...
If threadId is not in the inputs, generate a unique identifier for this conversation and output it as "threadId" and set isNewThread to true
...
_Step 3: Respond to the user on the same channel as the inbound message (email example here)_
...
Use /Sendgrid to send an email to the user with the agent response, include the threadId at the end of the email on a new line as "Ref: [step1 threadId]"
```
...
Note that where we have FILL IN, you should not fill in the values - they are placeholders for the AI Agent to fill in. The only part of this example template that is not a note for the AI Agent is YOUR_TRIGGER_API_KEY and YOUR_TRIGGER_ID. These you would actually fill in with your Pinkfish trigger API key and trigger ID.
...
In the case above, I've included the threadId in the email response. So that when the user responds to the email, we'll still have the threadId (and can extract it from the response in step 1). You could use the sender's email address as the threadId, however if the user cc's other people on the email and they reply, the sender email will change and you'll lose the thread.

In the case of SMS, use the user's phone number as the threadId. Group messages will not work unless you figure out how to attach a threadId to the group message - similar to email.
