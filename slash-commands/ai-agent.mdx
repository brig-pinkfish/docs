...
The format we like for thread IDs is `[userId]-[purpose]-[uniqueId]` - but really, it's entirely up to you. The most important thing is that you include the threadId in the response to the user and that they then send it back in their response (which is why using a phone number or email address is the simplest solution).
...
- Pro tip: upload your agent prompt as a file to the automation and then retrieve it in step one. This will make editing and replacing your prompt easier.
