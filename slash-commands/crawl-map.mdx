...
Discover all pages on a website by crawling from a starting URL. Useful for site analysis, content auditing, SEO research, or preparing for bulk scraping operations. Maps entire websites by following internal links and generates comprehensive URL lists.
...
- `include subdomains` - Include subdomain URLs in the map

- `ignore sitemap` - Skip using the sitemap for discovery

- `search` - Filter results to URLs containing specific keywords
...
```json
{
  ...
  "metadata": {
    "total_pages": "number of pages found",
    "crawl_time": "time taken"
  }
}
```
...
```
Map an entire website starting from the homepage.
...
```
Map website including all subdomains while utilizing the sitemap.
...
search: product
```
Find all pages containing "product" in the URL for targeted scraping.

### Notes

Supports subdomain inclusion/exclusion for comprehensive or focused mapping. Can utilize or ignore sitemap.xml for discovery strategy. Search filtering helps find specific page types or content areas. Respects robots.txt and crawling guidelines.
