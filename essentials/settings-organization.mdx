...
See [Pricing](/essentials/pricing) and [Credits](/essentials/credits) for the broader billing model.
...
Organization-wide skills work like [personal skills](/essentials/settings-personal#skills), but every member's agents can discover them. Use them to capture house style, compliance rules, or shared domain knowledge.
...
The page lists existing paths with their stages displayed as arrow-connected tags (`staging → prod`), plus a **Create Path** button. Creating a path opens a dialog where you name the path and add stages one at a time; each stage pairs a tag with a destination organization.

> Promotion Paths is feature-flag gated — it only appears in the sidebar when the feature is enabled for your organization. Ask your admin or Pinkfish support if you need access.
...
| Promotion Paths | — | Create / delete (when feature-flag enabled) | — |
