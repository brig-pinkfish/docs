...
Click **Continue** and Pinkfish opens a managed browser window pointing at the Login URL. **You drive the login yourself** — enter credentials, solve captchas, complete MFA, anything the service requires. When you land on an authenticated page, Pinkfish captures the session cookies and closes the window. The new login then appears on the Browser Login list.
...
- For services that *do* expose an API, prefer a regular [Connection](/essentials/connections) — API calls are faster, cheaper, and more reliable than browser automation.
