...
Coworker is your AI-powered assistant inside Pinkfish. Ask questions, delegate tasks to specialized agents, pull context from your knowledge bases, and interact with connected tools — all within a single chat interface. Coworker dynamically discovers available capabilities at runtime, so it always has access to the latest tools and integrations configured in your workspace.
...
Once selected, knowledge base results appear as tiles in the chat alongside Coworker's response. For more on creating and managing collections, see [Datastore](/essentials/datastore).
...
- Coworker uses the Claude Haiku model, optimized for fast response times.
- Knowledge base collection selection is locked after the first message in a chat. Start a new chat to switch collections.
- Coworker is channel-eligible, meaning it can be connected to Slack and Microsoft Teams workspaces.
