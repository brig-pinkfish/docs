...
<CardGroup cols={2}>
  <Card title="Browser Login Sessions" icon="browser" href="/essentials/browser-logins">
    Store authenticated browser sessions that can be reused across remote browser VMs. Automate services without APIs by maintaining logged-in sessions with 2FA, captchas, and complex authentication flows.
  </Card>
...
Unique capability to store authenticated browser sessions that persist across remote browser VMs. Automate services without APIs by handling complex authentication flows including 2FA, captchas, and multi-step logins—sessions are reusable across multiple workflow executions.
