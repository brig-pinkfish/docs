...
6. **Data Collection Triggers**: Automatically start your workflow when data changes in your Datastore or Filestore collections. Set triggers at the collection level (all items) or item level (specific items) for onCreate, onEdit, and onDelete events. Perfect for building reactive systems that respond to data changes. See [Structured Datastore](/slash-commands/datastore-structured), [Unstructured Datastore](/slash-commands/datastore-unstructured), and [Filestorage](/slash-commands/filestorage) documentation for detailed usage.
