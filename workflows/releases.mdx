...
<Warning>
  You cannot release a workflow until it has at least one successful run in **live** mode — that is, a run where no nodes were in [mock mode](/workflows/mock-mode). The Release button is disabled until this condition is met.
</Warning>
...
<Frame>
  <img src="/images/workflows/releases/release-dropdown.png" alt="Release dropdown open with the Publish a Release panel and a Publish CTA." />
</Frame>
...
<Frame>
  <img src="/images/workflows/releases/restore-version-dropdown.png" alt="Restore Version dropdown open with V2 (current) marked and a V1 entry with a Restore button, each labeled with a timestamp." />
</Frame>
...
{/* TODO screenshot: Version Preview Badge shown on the canvas while a past release is previewed. Capture blocked on this routine run — clicking the past-version row did not enter preview mode; confirm the UI flow with engineering before re-attempting. */}
...
Copy the public URL with the **Copy** button next to it. Anyone with the URL hits your workflow's [interface](/workflows/interface-builder) (if you defined one) or the API endpoint.

<Frame>
  <img src="/images/workflows/releases/public-template-section.png" alt="Release dropdown open showing the Release History entry with the Make Available as Public Template toggle switched on, the generated public URL, and a Copy button." />
</Frame>
...
<CardGroup cols={2}>
  ...
  <Card title="Interface" icon="window-maximize" href="/workflows/interface-builder">
