...
The **Monitor** tab inside the Agent Builder is a focused debugger for *this* agent. Where the [org-wide Agent Monitor](/orchestration/monitor) gives you every chat across every agent, the in-builder Monitor only shows chats for the agent you're editing.
...
| Use the in-builder **Monitor** tab when… | Use the [org-wide **Agent Monitor**](/orchestration/monitor) when… |
|------|------|
...
- [Agent Monitor](/orchestration/monitor) — the org-wide version.
- [Tools tab](/agents/connect-tools) — where you change what the agent can call once the Monitor tells you it's calling the wrong thing.
