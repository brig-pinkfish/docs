...
<img src="/images/agents/tools-tab.png" alt="Tools tab showing the Search servers… input, a Show Selected button, and sections for Connections, Embedded Services, Workflows, and Auto-selected Tools with a long list of MCP servers like activecampaign, activetrail, adobe-analytics, adobe-experience-manager, adobe-sign, airtable, alarmio, amazon-s3, and more" />
...
Some tools can be configured to pause for explicit user approval before running. Unlike fixed "always allow / ask / never" policies, Pinkfish approvals are **interactive at runtime**: the agent drafts a call, the platform creates a pending approval, and a human clicks **Approve** or **Reject** before execution continues.
...
- [API connections](/essentials/api-connections) — the integrations that feed the Connections section.
