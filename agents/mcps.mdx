...
In order for your agent to make use of authenticated MCP servers/tools, you'll first need to authenticate to the given service in Integrations. After you add an integration, your agents will be able use the tools provided for that integration. For example, if you authenticate to Google Drive, your agents will be able to use the Google Drive tools such as list files and write files. The agent acts like your proxy, so it only has access to integrations to which you already have permission. When you share a custom agent, the integrations do not share along with it. So if you create a custom agent and share it, users of that agent will need to use their own integrations (or you'll need to share your integration with them as well).
