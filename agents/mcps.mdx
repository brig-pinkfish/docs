...
Here's the complete list of available servers. Within the product, use the "Examples and Tools" menu from the agent prompt box to see tools per server.
...
2. **Custom Agent Configuration**: When building custom agents, select the MCP servers you want to include
3. **Tool Discovery**: Each server provides specific tools that can be discovered and used by your agents
